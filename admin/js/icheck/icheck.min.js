...
(function(win, doc, $) {
  ...
  if (!win.ichecked) {
    win.ichecked = function() {
      ...
      // detect mobile users
      var isMobile = /mobile|tablet|phone|ip(ad|od)|android|silk|webos/i.test(
        ua
      );

      // setup events
      var mouse = ["mouse", "down", "up", "over", "out"]; // bubbling hover
      var pointer = win.PointerEvent
        ? ["pointer", mouse[1], mouse[2], mouse[3], mouse[4]]
        : ["MSPointer", "Down", "Up", "Over", "Out"];
      var touch = ["touch", "start", "end"];
      var noMouse = (isTouch && isMobile) || isPointer;

      // choose events
      var hoverStart = noMouse
        ? isTouch ? touch[0] + touch[1] : pointer[0] + pointer[3]
        : mouse[0] + mouse[3];
      var hoverEnd = noMouse
        ? isTouch ? touch[0] + touch[2] : pointer[0] + pointer[4]
        : mouse[0] + mouse[4];
      var tapStart = noMouse
        ? isTouch ? false : pointer[0] + pointer[1]
        : mouse[0] + mouse[1];
      var tapEnd = noMouse
        ? isTouch ? false : pointer[0] + pointer[2]
        : mouse[0] + mouse[2];
      ...
      var styleList;
      var styleArea =
        defaults.areaStyle !== false
          ? 'position:absolute;display:block;content:"";top:#;bottom:#;left:#;right:#;'
          : 0;
      var styleInput =
      ...
      var style = function(rules, selector, area) {
        if (!styleTag) {
          ...
          // append to header
          (doc.head || doc.getElementsByTagName("head")[0]).appendChild(
            styleTag
          );
        ...
        if (!selector) {
          selector =
            "div." +
            (area
              ? areaClass + area + ":after"
              : divClass + " input." + nodeClass);
        }
      ...
      var tidy = function(node, key, trigger, settings, className, parent) {
        if (hashes[key]) {
          ...
          if (parent.length) {
            // input
            $(node)
              .removeClass(nodeClass + " " + className)
              .attr("style", settings.style);

            // label
            $("label." + settings.esc).removeClass(
      ...
      var process = function(data, options, ajax, silent) {
        ...
        while (element--) {
          ...
          for (nodeDataProperty in nodeData) {
            ...
            nodeDataCache[
              nodeDataProperty.replace(
                /checkbox|radio|class|id|label/g,
          ...
          if (settings.init !== false && ~handle.indexOf(nodeType)) {
            ...
            while (labelsLength--) {
              ...
              if (labelKey) {
                ...
                labelString =
                  (!!labelString ? labelString + " " : "") + labelClass;
              }
                ...
                if (nodeAttrCache[nodeInheritItem] !== undefined) {
                  if (nodeInheritItem == "class") {
                    ...
                    styler.setAttribute(
                      nodeInheritItem,
                      nodeInheritItem == "id"
                        ? baseClass + "-" + nodeAttrCache[nodeInheritItem]
                        : nodeAttrCache[nodeInheritItem]
                    );
                  }
                }
              }
            ...
            if (area) {
              ...
              if (computed) {
                stylerStyle = computed(styler, null).getPropertyValue(
                  "position"
                );
              } else {
      ...
      var operate = function(node, parent, key, method, silent, before, ajax) {
        ...
        if ((!before || ajax) && method !== "click") {
          ...
          states.indeterminate = [
            node.getAttribute("indeterminate") == "true" ||
              !!node.indeterminate,
            "Indeterminate",
            "Determinate"
          ];
      ...
      $.fn.icheck = function(options, fire) {
        // detect methods
        if (
          /^(checked|unchecked|indeterminate|determinate|disabled|enabled|updated|toggle|destroy|data|styler)$/.test(
            options
          )
        ) {
      ...
      $(doc)
        ...
        .on(
          "click.i change.i focusin.i focusout.i keyup.i keydown.i",
          "input." + nodeClass,
          function(event) {
            var self = this;
            var key = extract(self.className);
            ...
            if (key) {
              ...
              var parent =
                emitter == "click"
                  ? false
                  : closest(self, "div", settings.className);
              var label;
              var states;
              ...
              converters: {
                "text html": function(data) {
                  if (win.icheck.autoAjax && body) {
                    ...
                    frameData = frame.contentDocument
                    ...
                    data = (
                      frameData.body ||
                      frameData.getElementsByTagName("body")[0]
                    ).innerHTML;
